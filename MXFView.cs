...
namespace Myriadbits.MXFInspect
{
    public partial class MXFView : MyFormPage
    {
        ...
        private void MXFView_Load(object sender, EventArgs e)
        {
            ...
            // Set tree offset column formatting
            SetOffsetStyleOfColumns(MXFInspect.Properties.Settings.Default.ShowOffsetAsHex);
        ...
        private void treeListViewMain_SelectionChanged(object sender, EventArgs e)
        {
            ...
            if (obj != null)
            {
                if (!m_fDoNotSelectOther)
                {
                    ...
                    ReadData(obj, Properties.Settings.Default.ShowOffsetAsHex);
                }
            }
        ...
        private void ReadData(MXFObject obj, bool showOffsetAsHex)
        {
            ...
            if (len > 0)
            {
                ...
                string offsetFormatter = showOffsetAsHex ? "0x{0:X8}" : "{0:D10}";
                for (int n = 0; n < lines; n++)
                {
                    ...
                    sb.AppendLine(string.Format(offsetFormatter + "  {1}  {2}", readerOffset + (n * dataLength), hex, asciisafe));
                    offset += dataLength;
                }
            }
        ...
        {
            SetOffsetStyleOfColumns(Properties.Settings.Default.ShowOffsetAsHex);
            SetOffsetStyleOfHexWindow(Properties.Settings.Default.ShowOffsetAsHex);
            this.treeListViewMain.Refresh();
            this.treeListViewLogical.Refresh();
        }

        private void SetOffsetStyleOfHexWindow(bool showOffsetAsHex)
        {
            MXFObject selMain = this.treeListViewMain.SelectedObject as MXFObject;
            MXFObject selLogical = this.treeListViewLogical.SelectedObject as MXFObject;
            
            if(selMain != null)
            {
                ReadData(selMain, showOffsetAsHex);
            }
            
            if(selLogical != null)
            {
                ReadData(selLogical, showOffsetAsHex);
            }
            
        }

        private void SetOffsetStyleOfColumns(bool showOffsetAsHex)
        {
            string offsetFormatter = showOffsetAsHex ? "0x{0:x}" : "";
            this.treeListViewMain.GetColumn(0).AspectToStringFormat = offsetFormatter;
            this.treeListViewLogical.GetColumn(0).AspectToStringFormat = offsetFormatter;
            this.treeListViewMain.RebuildColumns();
            this.treeListViewLogical.RebuildColumns();
        }
    }
}
